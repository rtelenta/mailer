...
- `FROM_ADDRESS` MUST be exported from `lib/constants.ts`; add `.env.example` entry if one exists
- All rate-limit and delivery logic MUST reside in the API handler (`lib/api/templates.ts` or a dedicated `lib/api/templateTestSend.ts` sub-router)
